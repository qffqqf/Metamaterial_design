...
source_func = exp(-((z-1*L/3)**2 + (x-Lx/2)**2 + (y-Ly/2)**2)/(L/20)**2)
...
u_vis = mesh.MaterialCF({"solid": gfu_u}, default=(0,0,0))
p_vis = mesh.MaterialCF({"fluid": gfu_p}, default=0)

# Draw(u_vis, mesh, "u_masked")
Draw(gfu_p, mesh, "p_masked")
