...
class HybridSystem:
    ...
    def combine_matrices(self, Z_fem, Z_hyb, Z_wbm, s_hyb, s_wbm):
        ...
        s_fem_free = s_hyb[self.free_indices]

        # Block Matrix 
        top_row = sp.hstack([Z_fem_free, Z_hyb_free])
        bottom_row = np.hstack([Z_hyb_free.T, Z_wbm])
        Global_Matrix = sp.vstack([top_row, bottom_row]).tocsc()

        # Global RHS Vector 
        Global_RHS = sp.vstack([sp.csr_matrix(s_fem_free).reshape(-1, 1), sp.csr_matrix(s_wbm).reshape(-1, 1)]).tocsc()

        if isinstance(Global_RHS, sp.spmatrix):
            print("Global_Matrix is a SciPy sparse matrix.")
        else:
            print("Global_Matrix is NOT a SciPy sparse matrix (likely dense).")
