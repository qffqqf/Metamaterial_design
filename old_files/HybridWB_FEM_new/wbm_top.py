...
class WBM_Top:
    ...
    def _generate_wave_functions(self):
        ...
        self.kz_0 = - np.sqrt(self.k**2 - self.k_x0**2 - self.k_y0**2)
        self.wave_par = exp(1j * (self.kz_0 * z))
    ...
    def assemble_matrices(self, mesh, fes, test_function_v, interface_name="top"):
        ...
        d_wave_par_dz = self.wave_par * 1j * self.kz_0
        hyb_rhs += (d_wave_par_dz) * test_function_v * ds(interface_name)
        hyb_rhs.Assemble()
        ...
        for i in range(self.total_waves):
            ...
            s_wbm[i] = Integrate(- (dphi_i_dz) * self.wave_par, mesh, definedon=mesh.Boundaries(interface_name))
            with TaskManager():
                cwf.Assemble()
